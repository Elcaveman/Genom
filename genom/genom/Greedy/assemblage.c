...
void insertion_liste(Liste *liste, char new_ligne[]){
...
void supprimer(Liste *liste, char ligne[]) {
	Element *current = liste->HEAD;
	Element *precedant;
...
void Assemblage_greed(char input_file[200] ,char output_file[200]){

    Liste* listeDesSequences = NULL;
    listeDesSequences = chargement_input(input_file);

    if (listeDesSequences==NULL){
        g_printf("Impossible de charger le fichier");
        return;
    }
    ...
    sauvegarde_superstring(output_file, superString);
